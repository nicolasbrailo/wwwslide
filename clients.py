...
class Clients:
    ...
    def __init__(self, conf, flask_app, albums, img_sender):
        ...
        flask_app.add_url_rule("/get_current_img_qr/<client_id>", "get_current_img_qr", self.get_current_img_qr)
    ...
    def _current_img_path(self, client_id):
        client_id = self._guess_or_register_client(client_id)
        cfg = self.known_clients[client_id]
        ...
        return cfg["imgs_queue"][cfg["imgs_queue_idx"]]
    ...
    def get_current_img_raw(self, client_id=None):
        imgpath = self._current_img_path(client_id)
        return self.img_sender.img_raw(imgpath)

    def get_current_img_qr(self, client_id=None):
        imgpath = self._current_img_path(client_id)
        return self.img_sender.img_qr_only(imgpath)
